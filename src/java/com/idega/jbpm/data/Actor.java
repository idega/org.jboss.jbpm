 ...
 * IC_PERMIT_ROLE table. If there are no permissions for actor with process instance id != null,
 * then the permissions for process name are taken (i.e. the permissions are specified for process
 * definition scope).
 *
...
@NamedQueries( {
        ...
        @NamedQuery(name = Actor.getAllProcessInstancesIdsHavingRoleName, query = "select distinct a."
                ...
                + Actor.processRoleNameProperty + " is not null"),
        @NamedQuery(name = Actor.getSetByPIIdHavingRoleName, query = "from Actor a where a."
                + Actor.processInstanceIdProperty
                + " = :"
                + Actor.processInstanceIdProperty
                + " and a."
                + Actor.processRoleNameProperty + " is not null"),
        @NamedQuery(name = Actor.getSetByPIIdAndNotContainingRoleNames, query = "from Actor a where a."
                + Actor.processInstanceIdProperty
                + " = :"
                + Actor.processInstanceIdProperty
                + " and a."
                ...
                + " not in (:"
                + Actor.processRoleNameProperty + ")"),
        @NamedQuery(name = Actor.getSetByPIIdsHavingRoleName, query = "from Actor a where a."
                + Actor.processInstanceIdProperty
                + " in(:"
                + Actor.processInstanceIdProperty
                + ") and a."
        ...
        @NamedQuery(name = Actor.getRoleNameHavingRightsModifyPermissionByPIId, query = "select a."
                + Actor.processRoleNameProperty
                + " from Actor a, in (a."
                + Actor.actorPermissionsProperty
                + ") permissions where a."
                + Actor.processInstanceIdProperty
                + " = :"
                + Actor.processInstanceIdProperty
                + " and permissions."
                + ActorPermissions.modifyRightsPermissionProperty
                + " = true and a."
                + Actor.processRoleNameProperty
                + " is not null"),
        @NamedQuery(name = Actor.getRolesNamesHavingCaseHandlerRights, query = "select a."
                + Actor.processRoleNameProperty
                + " from Actor a inner join a."
                + Actor.actorPermissionsProperty
                + " permissions where a."
                + Actor.processInstanceIdProperty
                + " = :"
                + Actor.processInstanceIdProperty
                + " and permissions."
                + ActorPermissions.caseHandlerPermissionProperty
                + " = true and a."
                + Actor.processRoleNameProperty
                + " is not null"),
        @NamedQuery(name = Actor.getRolesHavingCaseHandlerRights, query = "select a from Actor a inner join a."
                + Actor.actorPermissionsProperty
                + " permissions where a."
                + Actor.processInstanceIdProperty
                + " = :"
        ...
        @NamedQuery(name = Actor.getProcessInstanceIdsByUserIdentity, query = "select act."
                ...
                + "inner join act."
                + Actor.nativeIdentitiesProperty
                + " ni "
                + "where ni."
        ...
        @NamedQuery(name = Actor.getActorsByUserIdentityAndProcessInstanceId, query = "select act from Actor act "
                + "inner join act."
                + Actor.nativeIdentitiesProperty
                + " ni "
                + "where act."
        ...
        @NamedQuery(name = Actor.getTaskInstancesByUserRole, query = "select task_instance.id from org.jbpm.taskmgmt.exe.TaskInstance task_instance where task_instance."
...
"select act.process_instance_id as processInstanceId from " + Actor.TABLE_NAME
        + " act " + "where act.role_name in (:" + Actor.processRoleNameProperty
        + ") and act.process_instance_id is not null " + "union "
        + "select act.process_instance_id as processInstanceId from "
        + Actor.TABLE_NAME + " act " + "inner join "
        + NativeIdentityBind.TABLE_NAME + " ni "
...
public class Actor implements Serializable {
	...
	@OneToMany(mappedBy = NativeIdentityBind.actorProperty, cascade = {
	...
	public void addNativeIdentity(NativeIdentityBind ni) {
		List<NativeIdentityBind> nis = getNativeIdentities();

		if (nis == null) {
			nis = new ArrayList<NativeIdentityBind>();
			setNativeIdentities(nis);
		}
