...
//		meta data meant to store bpmUser related data
	...
	public BPMUser getLoggedInBPMUser(String bpmUserUUID, User usr) {
		BPMUser bpmUser = getLoggedInBPMUser(IWContext.getCurrentInstance(), bpmUserUUID, usr);
		return bpmUser;
	}
	...
	public BPMUser getLoggedInBPMUser(IWContext iwc, String bpmUserUUID, User usr) {
		if (bpmUserUUID == null) {
			if (iwc != null) {
				...
				if (bpmUserUUID == null)
					bpmUserUUID = (String) iwc.getRequest().getAttribute(BPMUserImpl.bpmUsrParam);
...
//		we resolve bpm user if it's not already resolved, or if it doesn't match the pk or uuid provided in the params
		if (bpmUsr.getBpmUser() == null || ((bpmUserPK != null && !bpmUsr.getBpmUser().getPrimaryKey().equals(bpmUserPK)) ||
				(!StringUtil.isEmpty(bpmUserUUID) && !bpmUsr.getBpmUser().getUniqueId().equals(bpmUserUUID)))) {
	...
	public boolean isAssociated(User realUsr, User bpmUsr, boolean autoAssociate) {
		...
		try {
			...
			if(roleName != null && roleName.length() != 0 && processInstanceIdStr != null && processInstanceIdStr.length() != 0) {
				...
				List<String> prn = new ArrayList<String>(1);
				prn.add(roleName);
				...
				if (proles != null && !proles.isEmpty()) {
					...
					for (NativeIdentityBind ni : nis) {
						if (!foundForRealUsr && ni.getIdentityType() == IdentityType.USER &&
								ni.getIdentityId().equals(realUsr.getPrimaryKey().toString())) {
							foundForRealUsr = true;
							continue;
						}
						if (!foundForBPMUsr && ni.getIdentityType() == IdentityType.USER &&
								ni.getIdentityId().equals(bpmUsr.getPrimaryKey().toString())) {
							foundForBPMUsr = true;
							continue;
						}
					}
